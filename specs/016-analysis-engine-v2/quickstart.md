...
- `pnpm build`: pass on 2026-06-02 after route-export cleanup for `POST /api/analysis/start` and follow-up TypeScript fixes in deposits, pools, enrichment, and materialization task modules.
...
- Focused Engine V2 reward/materialization regressions: pass on 2026-06-02 through `reward-accounting.test.ts`, `engine-v2-materialization.test.ts`, and `route.test.ts` after closing the documented basic-pool reward/fee attribution gaps and fixing the route module shape required by Next.js build-time validation.
...
- Signoff scope note: as of 2026-06-02 the repository-level signoff basis includes green build, green deterministic regression coverage, green focused reward/materialization regressions, and DB-only request-path guards. A fresh provider-backed rerun against the real wallet was not re-executed in this session, so this signoff is codebase/regression-backed rather than a new certification of live wallet output.
